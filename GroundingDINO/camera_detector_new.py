...
class CameraDetector:
    ...
    def detect_objects_simple(self):
        ...
        for det_idx, detection in enumerate(selected_detections):
            ...
            # === 新增：過濾 3D 點雲邊緣的極端值 (飛點雜訊) ===
            if len(points_3d) > 20:
                # 排除 XYZ 三個維度前後 2% 的極端值，避免單一雜訊撐大 Bounding Box
                p_lower = np.percentile(points_3d, 2, axis=0)
                p_upper = np.percentile(points_3d, 98, axis=0)
                
                valid_mask = (
                    (points_3d[:, 0] >= p_lower[0]) & (points_3d[:, 0] <= p_upper[0]) &
                    (points_3d[:, 1] >= p_lower[1]) & (points_3d[:, 1] <= p_upper[1]) &
                    (points_3d[:, 2] >= p_lower[2]) & (points_3d[:, 2] <= p_upper[2])
                )
            ...
            # self._save_point_cloud_plot(points_3d, bbox_3d, det_idx)
            self._save_point_cloud_plot(points_3d, bbox_3d, det_idx, self.latest_color_image)
        # 儲存結果
        vis_bgr = cv2.cvtColor(vis, cv2.COLOR_RGB2BGR)
        ...
        if not any_detected:
            ...
            self.clear_detection_data()
            return True
    # def _save_point_cloud_plot(self, points_3d, bbox_3d, det_idx, color_image=None):
    #     """新增：繪製並儲存 3D 點雲與邊界框的視覺化圖 (高密度 + 真實色彩)"""
    ...
    #     # --- 修改 1：降低降採樣比例，保留最多 40,000 個點 ---
    #     step = max(1, len(points_3d) // 30000)
    #     sub_points = points_3d[::step]
        
    #     # --- 取得點雲對應的真實色彩 ---
    #     if color_image is not None and self.intrinsics is not None:
    #         # 獲取相機內參
    #         fx, fy = self.intrinsics.fx, self.intrinsics.fy
    #         cx, cy = self.intrinsics.ppx, self.intrinsics.ppy
    ...
    #         u = (X * fx / Z_safe + cx).astype(int)
    #         v = (Y * fy / Z_safe + cy).astype(int)
    ...
    #         # 提取顏色 (OpenCV 預設為 BGR，需轉為 RGB 並正規化至 0~1 給 matplotlib 使用)
    #         point_colors = color_image[v, u]
    #         point_colors = point_colors[:, ::-1] / 255.0
    #     else:
    #         # 如果沒有傳入影像，退回原本的單一顏色 (Cyan)
    #         point_colors = 'c'
    #     # -----------------------------------
        
    #     # # --- 修改 2：畫出高密度點雲 (調整 marker, size, alpha, 移除邊框) ---
    #     # ax.scatter(sub_points[:, 0], sub_points[:, 1], sub_points[:, 2], 
    #     #            c=point_colors, marker='o', s=12, alpha=0.95, edgecolors='none')
        
    #     # # 畫出 3D Bounding Box (紅色邊框)
    #     # corners = bbox_3d['corners']
    #     # edges = [
    #     #     (0, 1), (0, 2), (0, 4), (1, 3), (1, 5),
    #     #     (2, 3), (2, 6), (3, 7), (4, 5), (4, 6),
    ...
    #     #     pt1 = corners[edge[0]]
    #     #     pt2 = corners[edge[1]]
    #     #     ax.plot([pt1[0], pt2[0]], [pt1[1], pt2[1]], [pt1[2], pt2[2]], 'r-', linewidth=2)
            
    #     # ax.set_xlabel('X (m)')
    #     # ax.set_ylabel('Y (m)')
    #     # ax.set_zlabel('Depth Z (m)')
    #     # ax.set_title(f'3D Point Cloud & Bounding Box (Detection #{det_idx+1})')
        
    #     # # 強制讓 XYZ 軸比例相同，看起來才不會變形
    ...
    #     # # ax.invert_zaxis()
    #     # # ax.invert_xaxis()
    #     # plot_path = os.path.join(self.output_dir, f"point_cloud_3d_{det_idx}.png")
    #     # plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    #     # plt.close(fig)
    #     plot_X = sub_points[:, 0]       # 相機 X 保持不變 (右)
    #     plot_Y = sub_points[:, 2]       # 把相機 Z (深度) 放到繪圖的 Y 軸 (平面的前後)
    #     plot_Z = -sub_points[:, 1]      # 把相機 Y 加負號 (朝上)，放到繪圖的 Z 軸 (高度)
    ...
    #         pt2 = corners[edge[1]]
    #         # 這裡的線條一樣要套用座標映射！
    #         ax.plot([pt1[0], pt2[0]],          # Plot X
    #                 [pt1[2], pt2[2]],          # Plot Y (對應相機 Z)
    #                 [-pt1[1], -pt2[1]], 'r-', linewidth=2) # Plot Z (對應相機 -Y)
    ...
    def _save_point_cloud_plot(self, points_3d, bbox_3d, det_idx, color_image=None):
        """新增：繪製並儲存 3D 點雲與邊界框的視覺化圖 (飽滿+轉正+論文級美化)"""
        if points_3d is None or len(points_3d) == 0:
            return
        ...
        # 降採樣點雲 (保留最多 40,000 個點以維持細節)
        step = max(1, len(points_3d) // 40000)
        sub_points = points_3d[::step]
        
        # 取得點雲對應的真實色彩 (此段保持向量化映射邏輯)
        if color_image is not None and self.intrinsics is not None:
            fx, fy = self.intrinsics.fx, self.intrinsics.fy
            cx, cy = self.intrinsics.ppx, self.intrinsics.ppy
        ...
        # 座標映射 (轉正視角，沿用 Turn 10 修正)
        plot_X = sub_points[:, 0]       # 相機 X 保持不變
        plot_Y = sub_points[:, 2]       # 把相機 Z (深度) 放到繪圖的 Y 軸
        plot_Z = -sub_points[:, 1]      # 把相機 Y 加負號 (朝上)，放到繪圖的 Z 軸
        
        # --- [關鍵修復] 調整點的渲染參數，讓點雲變清晰 ---
        # 1. 把 marker 改回 'o' (圓點)，這比 '.' 強烈很多。
        # 2. 把尺寸 (s) 從 3 大幅拉高到 8。在 4 萬點密度下，點視覺上會融合，讓物件看起來變實心。
        # 3. 把 alpha (透明度) 調低一點 (0.9)，這能增加深度層次感，讓深色點雲更立體。
        ax.scatter(plot_X, plot_Y, plot_Z, 
                   c=point_colors, marker='o', s=8, alpha=0.9, edgecolors='none')
        ...
        for edge in edges:
            ...
            ax.plot([pt1[0], pt2[0]],          # Plot X
                    [pt1[2], pt2[2]],          # Plot Y (對應相機 Z)
                    [-pt1[1], -pt2[1]], color='#FF1493', linewidth=1.5, alpha=0.85) # Plot Z (對應相機 -Y)
            
        ax.set_xlabel('Camera X (m)', labelpad=10)
        ax.set_ylabel('Depth Z (m)', labelpad=10)
        ax.set_zlabel('Height -Y (m)', labelpad=10)
        ax.set_title(f'3D Point Cloud & Bounding Box (Detection #{det_idx+1})', pad=15)
