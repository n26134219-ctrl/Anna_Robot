...
def assign_object_phase_callback(msg): #0224
    """接收 JSON 格式的物體名稱並更新"""
    try:
        # 將接收到的 JSON 字串轉為 Python 物件 (例如 list)
        # decoded_data = json.loads(msg.data)
        visual_object_names = json.loads(msg.data)
        # 2. 確保 decoded_data 是一個清單，如果對方只傳 "rice food" (字串)，我們把它變清單
        if isinstance(visual_object_names, str):
            visual_object_names = [visual_object_names]
        print(f"✅ 解析後的物體名稱列表: {visual_object_names}") 
        rospy.loginfo(f"收到指定物體名稱指令: {visual_object_names}")
        ...
        rospy.logerr("接收到的資料不是有效的 JSON 格式")
    # head_camera_capture()
"""接收轉換後的基座標"""
...
def command_callback(msg):
    ...
    if command == "capture_head":
        ...
        rospy.loginfo("正在關閉相機系統...")
        system.cleanup()
        rospy.signal_shutdown("Camera system closed by command")
    else:
